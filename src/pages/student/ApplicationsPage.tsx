  ...
  return (
    <DashboardLayout>
      <div className="space-y-4 sm:space-y-6 max-w-6xl">
        <div>
          <h1 className="text-xl sm:text-2xl font-heading font-bold text-foreground">My Applications</h1>
                  ...
                  {applications.map(app => (
                    <div key={app.id} className="p-3 rounded-lg border bg-card space-y-2">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{app.course_duration} • {new Date(app.created_at).toLocaleDateString()}</p>
                      </div>
                      ...
                      <div className="flex gap-2">
                        {app.payment_status === 'pending' && (
                          <Button size="sm" variant="outline" className="flex-1 text-xs" onClick={() => handleUploadClick(app.id)}>
                            <Upload className="h-3 w-3 mr-1" /> Upload Payment
                          </Button>
                        )}
                        {app.admission_letter_url && (
                          <Button size="sm" variant="outline" className="flex-1 text-xs text-success" asChild>
                            <a href={app.admission_letter_url} target="_blank" rel="noopener noreferrer">
                              <Download className="h-3 w-3 mr-1" /> Admission Letter
                            </a>
                          </Button>
                        )}
                      </div>
